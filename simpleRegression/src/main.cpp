...
int main(int argc, char ** argv) {
    ...
    while((option = getopt(argc, argv, "s:p:l:")) != -1){
        switch (option) {
            ...
            default: std::cout << "Unrecognised option. Valid options are \'-s seed\' \'-p paramFile.json\' \'-logs logs Folder\'." << std::endl; exit(1);
        }
    }
...
#define PRINT_ALL_DOT 1
		...
		if(i % 1 == 0){
			char buff[250];
			sprintf(buff, "%s/out_lastGen.%" PRIu64 ".p%d.dot", logsFolder, seed, indexParam);
